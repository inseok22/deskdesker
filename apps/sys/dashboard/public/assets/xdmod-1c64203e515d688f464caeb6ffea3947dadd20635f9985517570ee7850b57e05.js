...
var jobHelpers = {
  ...
  start_time: function() {
    return new Date(this.start_time_ts * 1e3).toLocaleString();
  },
  end_time: function() {
    return new Date(this.end_time_ts * 1e3).toLocaleString();
  },
  walltime: function() {
  ...
  efficiency_label: function(efficiencyValue, inverse = false) {
    ...
    return new import_handlebars.default.SafeString(label);
  }
};
      ...
      window.addEventListener("message", function(event) {
        if (event.origin !== xdmodUrl()) {
          ...
          if (event.data.action == "loginComplete") {
            ...
            reject2(new Error(`XDMoD Login iFrame at URL ${iframe && iframe.src} posted error message with info ${event.data.info}`));
          }
        }
      }, false);
    });
    fetch(xdmodUrl() + "/rest/auth/idpredirect?returnTo=%2Fgui%2Fgeneral%2Flogin.php").then((response) => response.ok ? Promise.resolve(response) : Promise.reject(new Error("Login failed: IDP redirect failed"))).then((response) => response.json()).then(function(data) {
      return new Promise(function(resolve2, reject2) {
        ...
        xdmodLogin.onerror = function() {
          reject2(new Error("Login failed: Failed to load XDMoD login page"));
        };
      });
    }).then(() => {
      return Promise.race([promise_to_receive_message_from_iframe, new Promise(function(resolve2, reject2) {
        setTimeout(reject2, 5e3, new Error("Login failed: Timeout waiting for login to complete"));
      })]);
    }).then(() => {
      resolve();
    }).catch((e) => {
...
var promiseLoggedIntoXDMoD = function() {
  return import_lodash.default.memoize(function() {
    return fetch(xdmodUrl() + "/rest/v1/users/current", { credentials: "include" }).then((response) => {
      if (response.ok) {
        return Promise.resolve(response.json());
      } else {
        return promiseLoginToXDMoD().then(() => fetch(xdmodUrl() + "/rest/v1/users/current", { credentials: "include" })).then((response2) => response2.json());
      }
    }).then((user_data) => {
...
function renderCoreHoursEfficiency(context) {
  const newContext = import_lodash.default.merge(context, { unit: "core hours", unit_title: "Core Hours" });
  const templateSource = $("#job-efficiency-template").html();
  const template = import_handlebars.default.compile(templateSource);
 ...
 */
//# sourceMappingURL=/pun/sys/dashboard/assets/xdmod.js-ffa5bbef5bc9b7c182d67abc3d72b5dd3370e4870b4fe58f14b375bab233076f.map
//!
;
