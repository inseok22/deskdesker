...
var icons_default = ALL_ICONS;

// app/javascript/icon_picker.js
var ICON_SHOW_ID = "product_icon";
var ICON_SELECT_ID = "product_icon_select";
var hiddenIcons = false;
...
function populateList() {
  ...
  if (list.length == 0 || icons_default.length == 0) {
    return;
  }
  const listContent = icons_default.map((name) => {
    return listItem(name);
  }).join("");
  list.html(listContent);
  icons_default.forEach((name) => {
    $(`#${iconId(name)}`).on("click", (event) => {
      picked(event);
    });
  });
}
...
function createInvertedIndex(arr) {
  const invertedIndex2 = {};
  arr.forEach((str, index) => {
    for (let i = 0; i < str.length; i++) {
      const char = str[i];
      if (!invertedIndex2[char]) {
        invertedIndex2[char] = [];
      }
      invertedIndex2[char].push(index);
    }
  });
  return invertedIndex2;
}
var invertedIndex = createInvertedIndex(icons_default);
function searchIcons(event) {
  ...
  if (searchString.length === 0) {
    for (let i = 0; i < icons_default.length; i++) {
      resultIndices.add(i);
    }
  } else {
    searchCharacters.forEach((char) => {
      const indices = invertedIndex[char];
      indices.forEach((index) => {
        const iconStr = icons_default[index].toLowerCase();
  ...
  icons_default.forEach((name, idx) => {
...
//# sourceMappingURL=/pun/sys/dashboard/assets/icon_picker.js-e78f230552c2d71057a0087ec4fbb12190a36702c71b6a9c82e543362a9c2305.map
//!
;
