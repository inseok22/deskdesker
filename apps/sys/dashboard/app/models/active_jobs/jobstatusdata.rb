module ActiveJobs
  ...
  class Jobstatusdata
    ...
    def extended_data_slurm(info)
      return unless info.native
      attributes = []
      attributes.push Attribute.new "클러스터", self.cluster_title
      attributes.push Attribute.new "작업 ID", self.pbsid
      attributes.push Attribute.new "작업 이름", self.jobname
      attributes.push Attribute.new "사용자", self.username
      attributes.push Attribute.new "계정", self.account
      attributes.push Attribute.new "파티션", self.queue
      attributes.push Attribute.new "상태", info.native[:state]
      attributes.push Attribute.new "사유", info.native[:reason]
      attributes.push Attribute.new "전체 노드", info.native[:nodes]
      attributes.push Attribute.new "노드 목록", self.nodes.join(", ") unless self.nodes.blank?
      attributes.push Attribute.new "전체 CPU", info.native[:cpus]
      attributes.push Attribute.new "시간 제한", info.native[:time_limit]
      attributes.push Attribute.new "사용 시간", self.walltime_used
      attributes.push Attribute.new "시작 시간", safe_parse_time(info.native[:start_time])
      attributes.push Attribute.new "종료 시간", safe_parse_time(info.native[:end_time])
      attributes.push Attribute.new "메모리", info.native[:min_memory]
